...
const defaults = {
  ...
  /**
   * Если true - будет сам отдавать изображения, если false - заменит адрес в заголовке :path
   */
  serveImages: true,
  /**
  ...
  /**
   * Опции библиотеки cwebp-bin
   */
  cwebpOptions: [],

  cwebpBinPath: '/usr/bin/cwebp',
};
...
export default class WebpMiddleware {
    ...
    return (request, response, next) => {
      if (this.__isSupportsWebp(request) && this.__checkMimeTypes(request)) {
        ...
        if (fs.existsSync(destPath)) {
          ...
          // Если файл ещё не создан - создаем его и отдаём в первый раз исходник
   ...
   * @private
   */
  async __createWebp(src, dest) {
    //fs.mkdirSync(path[, options])
    const destDir = path.dirname(dest);
    if (!fs.existsSync(destDir)) {
      fs.mkdirSync(destDir, {recursive: true});
    }
    const resCWebp = spawn(this.options.cwebpBinPath, [...this.options.cwebpOptions, src, '-o', dest]);
